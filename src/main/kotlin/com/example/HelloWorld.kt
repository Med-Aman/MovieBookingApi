...
@Controller
class HelloWorld(@Inject val repository : Repository) {
    ...
    fun getMovieListByTheater(
        @QueryValue theatername : String) : List<Theater> = repository.theaterList().filter { it.name.equals(theatername,ignoreCase = true) }
    ...
    fun getBookingDetails(
        ...
        val screen = repository.getScreenId(repository.getTheaterId(theater),
